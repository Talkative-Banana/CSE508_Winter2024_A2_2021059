...
def download_image(key_url):
    ...
    for link in range(len(urls)):
        filename = os.path.join(out_dir, '{}_{}.jpg'.format(key, link+1))
        ...
        try:
            response = request.urlopen(urls[link])
            image_data = response.read()
        except:
            print('Warning: Could not download image {} from {}'.format(key, urls[link]))
            return 1
